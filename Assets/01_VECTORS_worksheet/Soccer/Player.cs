...
public class Player : MonoBehaviour
{
    ...
    float AngleToPlayer()
    {
        ...
        Vector3 B = OtherPlayer.transform.position - transform.position;
        ...
        float angle = (float)Mathf.Acos(DotProd) * Mathf.Rad2Deg;
        Debug.Log(angle);
        return angle;
    ...
    void Update()
    {
        if (IsCaptain)
        {
            ...
            //float angle = Vector3.SignedAngle(Vector3.forward, OtherPlayer.transform.position, Vector3.forward);
            AngleToPlayer();
        }
    }
}
